...
- Keeps a local track list: add songs, delete them (with a confirmation, since deleting an already-transferred song invalidates everything after it), see what's already transferred — since the SP-1 itself has no way to report its own song list back
...
- Keeps transferring in the background: a foreground service holds the connection open through screen-off and app-switching, so you're not stuck staring at the screen for the ~10-minutes-per-minute-of-audio transfer time
...
- **Delete** asks for confirmation before removing a song — if it's already been transferred, the confirmation also warns that every song after it will need to be re-transferred once you delete this one, since storage on the SP-1 is sequential.
...
- You don't have to babysit the screen — a persistent notification shows progress while the transfer keeps running in the background.

#### While a transfer is running

- Every button except **Disconnect** disables itself for the duration — Scan, Connect, Check State, Add Song, and each song's own Delete button. This isn't just to keep things tidy: deleting a song or starting a second transfer while one is already writing to the device could corrupt what's being sent.
- Your progress is safe no matter what — every sector gets saved to the app's own memory the moment it's confirmed written, not just at the end. If the app closes, the phone dies, or the cable comes loose, whatever finished is still marked done next time you open the app, and Transfer Album will pick up from there.
- To stop a transfer on purpose, tap **Disconnect**. It won't cut off mid-write — it finishes whatever sector is currently in progress, saves that progress, and *then* disconnects cleanly.
...
- **Check Device State** — queries the SP-1's current state and, if it's sitting in boot mode, switches it into transfer mode (step 6 above).
...
- If a transfer gets interrupted (cable unplugged, app closed), the SP-1 can seem unresponsive with audio disabled. This is normal — long-press the SP-1's function button to restart it, then reconnect and run **Transfer Album** again. It will pick up any unfinished song from where it left off.
